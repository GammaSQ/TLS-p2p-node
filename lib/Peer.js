...
var Host = function Host(host, port) {
  ...
  //Problem: x isinstanceof tls.socket doesn't work.
  ...
  } else if (!fallback && host instanceof tls.TLSSocket){
    _socket = host;
    _host = _socket.remoteAddress;
    _port = _socket.remotePort;
    _version = net.isIP(_host);
    _TLS = true;
  } else if (fallback && host.__proto__ === target_proto){
    _socket = host;
    _TLS = true;
...
Peer.prototype.connect = function connect(connection) {
  ...
  //Check weather we have a pulse ...
  if(connection && isAlive(connection)){
    // ... and claim it as our own ...
    //(by defining it as property later on.)
    this.state = 'connected'
  } else {
    // ... or build our very own Frankenstein! (\'s Monster)
    host = connection? Host(connection) : this.host;
    // If we use TLS, we need some further instructions!
    options = settings.useTLS? settings.TLS_connection_options : {};
    options.host = host.host;
    options.port = host.port;
    // It COULD be a net-socket.
